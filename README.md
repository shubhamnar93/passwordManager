...
4. `main.py`: it setup entire GUI, when `Generate Password` button is clicked it calls `generate_password` method from `PasswordGenerator` class and insert the password, when `add` button is clicked it calls `save_password` method from `PasswordManager` class and shows the error window if any of the feilds are empthy, when `search` button is clicked it calls `search_password` method from `PasswordSearcher` class and show message box when method do not return false 
